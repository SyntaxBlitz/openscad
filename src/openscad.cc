...
// TODO see if these imports are still needed
...
bool checkAndExport(const std::shared_ptr<const Geometry>& root_geom, unsigned dimensions,
                    ExportInfo& exportInfo, bool is_stdout, ExportedHeapData* output, const std::string& filename)
{
...
)
{
  ...
  if (export_format == FileFormat::CSG) {
    ...
    if (dim > 0 && !checkAndExport(root_geom, dim, exportInfo, cmd.is_stdout, output, filename_str)) {
      return 1;
    }
